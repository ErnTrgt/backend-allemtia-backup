        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Blog Düzenle: {{ $blog->title }}</h3>
                            ...
                            <div class="col-md-8">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" 
                                              id="description" name="description" rows="3">{{ old('description', $blog->description) }}</textarea>
                                    @error('description')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="status" name="status">
                                        <option value="1" {{ old('status', $blog->status) == 1 ? 'selected' : '' }}>Aktif</option>
                                        <option value="0" {{ old('status', $blog->status) == 0 ? 'selected' : '' }}>Pasif</option>
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    @if($blog->blog_img)
                                        <img src="{{ asset('storage/' . $blog->blog_img) }}" alt="Mevcut görsel" 
                                             style="max-width: 100%; margin-bottom: 10px;">
                                    @else
                                        <p class="text-muted">Görsel yok</p>
                                    @endif
                                    <img id="preview" src="#" alt="Yeni önizleme" style="display: none; max-width: 100%; margin-top: 10px;">
                                </div>
                            </div>
                        </div>
                    </div>
